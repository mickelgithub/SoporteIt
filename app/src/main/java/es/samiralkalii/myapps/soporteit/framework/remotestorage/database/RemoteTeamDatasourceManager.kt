...
class RemoteTeamDatasourceManager(val fstore: FirebaseFirestore): IRemoteTeamManagementDatasource {
    ...
    override suspend fun createNewGroup(group: MutableMap<String, Any>) {
        val newGroupRef= fstore.collection(GROUPS_REF).document()
        group[KEY_ID]= newGroupRef.id
        newGroupRef.set(group)
    }

    override suspend fun updateGroup(groupId: String, group: MutableMap<String, Any>) {
        group[KEY_ID]= groupId
        fstore.collection(GROUPS_REF).document(groupId).set(group)
    }
